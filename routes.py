...
from app.mock_data import MOCK_RESOURCES, MOCK_COURSES
...
# ====== AI 术语表：从 CSV 加载并按拼音首字母 A-Z 分组 ======
def get_letter_pinyin(term: str) -> str:
    """
    返回 A-Z 分组字母：
    - 英文：取第一个英文字母
    - 中文：取拼音首字母
...
def load_terms_from_csv():
    """
    从 AICove/data/Google Machine Learning Glossary.csv 读取术语
    CSV 列名默认：term, definition
    """
    # current_app.root_path = AICove/app
    project_root = os.path.dirname(current_app.root_path)  # -> AICove/
    csv_path = os.path.join(project_root, "data", "Google Machine Learning Glossary.csv")
    ...
    terms.sort(key=lambda x: (x["letter"], x["term"].lower()))
    return terms


def group_terms_by_letter(terms):
    grouped = {chr(c): [] for c in range(ord("A"), ord("Z") + 1)}
    grouped["#"] = []
    for t in terms:
        grouped.setdefault(t["letter"], []).append(t)
    return grouped
# ====== /AI 术语表 ======
...
def resources():
    ...
    current_letter = (current_letter or 'A').upper()
    if current_letter not in list("ABCDEFGHIJKLMNOPQRSTUVWXYZ"):
        current_letter = 'A'
    ...
    # 1) 读取所有术语（全库）
    all_terms = load_terms_from_csv()

    # 2) ✅ 全库搜索（term / definition 命中）
    if q:
        q_lower = q.lower()
        all_terms = [
            t for t in all_terms
            if q_lower in (t.get('term', '').lower())
            or q_lower in (t.get('definition', '').lower())
        ]

    # 3) 再按当前字母筛选（搜索后再筛选字母）
    filtered_terms = [t for t in all_terms if t.get('letter') == current_letter]

    # 4) 分页
    total = len(filtered_terms)
    total_pages = max(1, (total + per_page - 1) // per_page)
    page = max(1, min(page, total_pages))  # 防止越界

    start = (page - 1) * per_page
    end = start + per_page
    paged_terms = filtered_terms[start:end]

    # 5) 当前页只需要一个分组（就是 current_letter）
    letters = list("ABCDEFGHIJKLMNOPQRSTUVWXYZ")
    terms_by_letter = {ch: [] for ch in letters}
    terms_by_letter[current_letter] = paged_terms

    # 推荐阅读
    resources = sorted(MOCK_RESOURCES, key=lambda x: x['created_at'], reverse=True)
    # 推荐课程（新增）
    courses = sorted(MOCK_COURSES, key=lambda x: x['created_at'], reverse=True)
    ...
    return render_template(
        ...
        resources=resources,
        courses=courses,   
        q=q  # ✅ 关键：把 q 传给模板，分页/A-Z 链接才能带上 q
    )
